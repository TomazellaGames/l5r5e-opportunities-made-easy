...
export class OpportunitiesApp extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  static open(message) {
    let app = OpportunitiesApp.#instances.get(message.id);
    if (!app) {
      app = new OpportunitiesApp(message);
      OpportunitiesApp.#instances.set(message.id, app);
    }
  ...
  _onRender(_context, _options) {
    ...
    this.element.querySelectorAll(".opp-tab").forEach(tab => {
      tab.addEventListener("click", () => {
        ...
        this.element.querySelectorAll(".opp-tab").forEach(t =>
          t.classList.toggle("active", t.dataset.tab === id));
        this.element.querySelectorAll(".opp-panel").forEach(p =>
